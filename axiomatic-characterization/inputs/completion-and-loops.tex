\begin{Uebung}{(Clark's Completion and Loop Formulas)}
For the following normal programs~$\Pi$, 
find the Clark's Completion $\Comp{\Pi}$,
the positive dependency graph $\G{\Pi}$,
the loops in $\Loop{\Pi}$,
the loop formulas in $\LF{\Pi}$,
all models of $\Comp{\Pi}$ and 
all the stable models of~$\Pi$.
%
\begin{UList}
\item
\(
